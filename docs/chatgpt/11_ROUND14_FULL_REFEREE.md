# Round 14 brief — Full harsh-referee pass + consistency sweep (whole manuscript)

**Repo:** https://github.com/Coucou2016/river-carbon-transport (branch `main`, commit pinned at send time)
**Text below is the full current manuscript (post-Rounds 10-13), extracted from `paper.md`.** Figure images are omitted; captions remain.

**State after Rounds 10-13 (all code-verified and merged):**
- Round 10 removed editorial scaffolding, merged Discussion to four subsections, rewrote Conclusions, and calibrated hedging via Phrasebank.
- Round 11 rewrote Abstract and Introduction in Fraehr rhythm.
- Round 12 expanded Methods 2.1-2.8 with code-verified mechanics, renumbered equations (1)-(5), and disclosed two P0 implementation limitations (training-target unit inconsistency; Damkoehler construction) plus the pre-fold k_need construction.
- Round 13 repaired claims-vs-table citations (RF subgroup values, date-grouped sensitivity, implied-source statistics), calibrated causal language in 3.2/3.5/4.1/4.2/4.3, and added six transition sentences.

## What we ask of ChatGPT

You are a HARSH reviewer for Environmental Modelling & Software (and, secondarily, WRR). The science, numbers, and conclusions are FROZEN: do not propose rerunning experiments or changing results. Propose only prose-level fixes. No em-dashes, no bold in proposed prose. Answer Q14.1-Q14.4 in English.

### Q14.1 Harsh referee report
Give a full referee report on the whole manuscript: recommendation (accept/minor/major/reject), 3-6 major points, 5-10 minor points, and a short list of what NOT to change. Be specific with section references.

### Q14.2 Consistency sweep
Check the whole manuscript for: (a) one metric one precision (e.g., is 0.0284 always four decimals? is 3.24 vs 3.244 consistent with its table? is 98.1 vs 98.096 consistent across body and tables?); (b) unit notation consistency (mol m-3 vs mol/m^3, m d-1 vs m/d, superscripts vs carets across Abstract/body); (c) acronym first-use (S_sgs, k_emp, k_eff, RMSE, MAE, MLP, LASSO, NHD, DOC, DIC, PLS); (d) tense parallelism within and across sections; (e) every figure/table cross-reference resolving to an existing caption; (f) term drift (Baseline capitalization, Residual-AI, k-correction, grouped vs nested cross-validation, practical equifinality). List each finding with the exact location and the exact correction.

### Q14.3 AI-marker / report-style check
List any remaining sentences that sound machine-generated, report-like, or defensive (bullet cadence, "It is worth noting", "In summary", hedge stacking, repetitive sentence openers, formulaic transitions). Give the replacement for each.

### Q14.4 Pre-submission gap list
Given the known outstanding items (authors, affiliations, Gao DOI, alkalinity/N/P/PAR covariates, width-sensitivity table, fold-level RMSE, C_eq appendix), state which of them would block an EMS submission and which are acceptable as supporting-information gaps. Do not invent values.
...
The primary result is negative. Under leave-one-reach-out transport-coupled evaluation, neither residual closure improves on the Baseline (Tables 2 and 3; Figure 3). The held-out C_aq RMSE is 0.0284 mol m-3 for the Baseline, 0.0573 for the Residual-AI multilayer perceptron, and 0.0745 for the random forest. The corresponding MAE values are 0.0132, 0.0326, and 0.0301, and the residual closures show positive concentration bias (0.0177 and 0.0180) where the Baseline bias is -0.0132. The date-grouped sensitivity reported in the repository metrics tables (Data availability) gives the same ordering (0.0284, 0.0591, and 0.0747).
...
Because residual-source learning did not improve held-out prediction, we next tested whether reallocating discrepancy to the gas-transfer term changed the concentration error.
...
Having established compensation between alternative closure terms at the sampled scale, we next examine whether the diagnosed residual itself changes with spatial coarse-graining.
...
The observed scale dependence raises a separate question of whether the residual can nevertheless be summarized by a compact dimensionless relation.
...
The sparse dimensionless closure provides a counterpoint to the flexible learners. It identifies a limited set of candidate dependencies, with Froude number, slope, and relative depth surviving selection, yet its compactness does not transfer into held-out skill: RMSE remains above the Baseline and the S* reconstruction fails under reach holdout. Compact forms are therefore not automatically validated or predictive. Under the present protocol, the tested sparse Pi-group representation does not provide cross-reach predictive utility; it remains useful as a diagnostic simplification.
...
End of Round 14 context. Answer Q14.1-Q14.4.
